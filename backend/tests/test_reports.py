def test_submit_report_classifies_infrastructure(client):
    resp = client.post(
        "/api/reports/",
        json={"text_note": "broken street light", "latitude": 60.2, "longitude": 24.6},
    )
...
def test_submit_report_defaults_to_other_and_flags_review(client):
    resp = client.post(
        "/api/reports/",
        json={"text_note": "trash everywhere", "latitude": 60.2, "longitude": 24.6},
    )
    body = resp.json()
...
def test_submit_report_missing_coordinates_returns_422(client):
    resp = client.post("/api/reports/", json={"text_note": "no coords"})
...
def test_submit_report_rejects_coordinates_outside_espoo_region(client):
    resp = client.post(
        "/api/reports/",
        json={"text_note": "not in Finland", "latitude": 40.7128, "longitude": -74.0060},
    )
    assert resp.status_code == 422
...
def test_submit_report_rejects_overlong_text_note(client):
    resp = client.post(
        "/api/reports/",
        json={"text_note": "x" * 1001, "latitude": 60.2, "longitude": 24.6},
    )
    assert resp.status_code == 422
...
def test_list_reports_returns_submitted_report(client):
    client.post(
        "/api/reports/",
        json={"text_note": "light broken", "latitude": 60.1, "longitude": 24.5},
    )
...
def test_submit_report_starts_in_submitted_status(client):
    resp = client.post(
        "/api/reports/",
        json={"text_note": "broken light", "latitude": 60.2, "longitude": 24.6},
    )
